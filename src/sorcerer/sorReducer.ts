...
export const sorReducer = (
  ...
  switch (action.type) {
    ...
    case "[SOR] REMOVE": {
      state = produce(state, (draft) => {
        switch (action.payload.type) {
          case "panel":
          case "command": {
            const index = draft.sorcerer.manifestInfo.entrypoints.findIndex(
          ...
          case "snippet": {
            const index = draft.sorcerer.snippets.list.findIndex(
              (item) => item.$$$uuid === action.payload.uuid,
            );
            if (index !== -1) {
    ...
    case "[SOR] SET_PANEL": {
      state = produce(state, (draft) => {
        const index = draft.sorcerer.manifestInfo.entrypoints.findIndex(
          (e) => e.type === "panel" && e.$$$uuid === action.payload.uuid,
        );
        if (index !== -1) {
    ...
    case "[SOR] SET_COMMAND": {
      state = produce(state, (draft) => {
        const index = draft.sorcerer.manifestInfo.entrypoints.findIndex(
          (e) => e.type === "command" && e.$$$uuid === action.payload.uuid,
        );
        if (index !== -1) {
          draft.sorcerer.manifestInfo.entrypoints[index] = {
            ...state.sorcerer.manifestInfo.entrypoints[index],
    ...
    case "[SOR] ASSIGN_SNIPPET_TO_PANEL": {
      state = produce(state, (draft) => {
        ...
        const index = state.sorcerer.manifestInfo.entrypoints.findIndex(
          (entryPoint) =>
            entryPoint.type === "panel" && entryPoint.$$$uuid === uuid,
        );

        if (index !== -1) {
          const found: IEntrypointPanel = draft.sorcerer.manifestInfo
            .entrypoints[index] as IEntrypointPanel;
          const foundState: IEntrypointPanel = state.sorcerer.manifestInfo
            .entrypoints[index] as IEntrypointPanel;
    ...
    case "[SOR] SET_HOST_APP": {
      state = produce(state, (draft) => {
        const found = draft.sorcerer.manifestInfo.host.find(
          (host) => host.app === action.payload.app,
        );
        if (found) {
