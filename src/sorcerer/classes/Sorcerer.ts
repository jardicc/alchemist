...
import {
  generateHtmlFileCode,
  generateScriptFileCode,
  getAllPanels,
  getManifestCode,
  getManifestGeneric,
} from "../sorSelectors";
...
const fs: uxp.storage.LocalFileSystemProvider =
...
export class SorcererBuilder {
  ...
  public static async buildPlugin(): Promise<void> {
    ...
    const manifestFile = await targetFolder.createFile("manifest.json", {
      overwrite: true,
    });
    ...
    const indexFile = await targetFolder.createFile("index.html", {
      overwrite: true,
    });
    const htmlContent = generateHtmlFileCode(state);
    await indexFile.write(htmlContent, {
      append: false,
      format: storage.formats.utf8,
    });

    const scriptFile = await targetFolder.createFile("index.js", {
      overwrite: true,
    });
    ...
    const zipFile = await folder.createFile(pluginName + ".ccx", {
      overwrite: true,
    });
    await zipFile.write(zipResult, {
      append: false,
  ...
  public static async exportPreset() {
    ...
    const file = await fs.getFileForSaving(
      "Sorcerer preset - " + (sor.manifestInfo.name || "") + ".json",
      {
        types: ["json"],
      },
    );
