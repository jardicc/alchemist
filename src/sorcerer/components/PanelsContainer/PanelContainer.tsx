      ...
      <div className="PanelContainerContainer" key="panelPanel">
        ...
        <div className="row">
          ...
          <SP.Textfield
            ...
            onInput={(e) =>
              onSet(activePanel.$$$uuid, { id: e?.target?.value ?? "" })
            }
          />
        </div>
        <div className="row">Assigned snippet(s):</div>
        <div className="column">
          {snippets.map((checkboxItem, index) => (
            <div className="row" key={checkboxItem.$$$uuid + "_" + index}>
              <SP.Checkbox
                key={checkboxItem.$$$uuid}
                checked={
                  activePanel.$$$snippetUUIDs.includes(checkboxItem.$$$uuid)
                    ? true
                    : undefined
                }
                onChange={(e) =>
...
const mapDispatchToProps = (dispatch: Dispatch): IPanelContainerDispatch => ({
  ...
  onAssignSnippet: (uuid, operation, snippetUuid) =>
    dispatch(assignSnippetToPanelAction(operation, uuid, snippetUuid)),
});

export const PanelContainer = connect<
  IPanelContainerProps,
  IPanelContainerDispatch,
  IOwn,
  IRootState
>(
  mapStateToProps,
