...
import {
  ...
  setMainAction,
  setSelectAction,
  TSetMainActionPayload,
  TSetPanelHostActionPayload,
} from "../../sorActions";
    ...
    const res = host.map((h, i) => (
      <div key={i} className="host">
        ...
        <div className="row">
          ...
          <SP.Textfield
            value={h.minVersion}
            onInput={(e) => onSetHost(h.app, { minVersion: e.target?.value })}
          />
        </div>
        <div className="row">
          API version:{" "}
          <SP.Textfield value={h.data.apiVersion.toString()} disabled={true} />
        </div>
      </div>
    ));
    ...
    return (
      <div className="GeneralContainerContainer" key="generalPanel">
        ...
        <div className="row">
          Manifest version:{" "}
          <SP.Textfield
            value={manifestGeneric.manifestVersion.toString()}
            disabled={true}
          />
        </div>
        <div className="row">
          Plugin name:{" "}
          <SP.Textfield
            value={manifestGeneric.name}
            onInput={(e) => onSetMain({ name: e.target?.value })}
          />
        </div>
        <div className="row">
          Plugin ID:{" "}
          <SP.Textfield
            value={manifestGeneric.id}
            onInput={(e) => onSetMain({ id: e.target?.value })}
          />
        </div>
        <div className="row">
          Main file:{" "}
          <SP.Textfield value={manifestGeneric.main} disabled={true} />
        </div>
        <div className="row">
          Plugin version:{" "}
          <SP.Textfield
            value={manifestGeneric.version}
            onInput={(e) => onSetMain({ version: e.target?.value })}
          />
        </div>
        <h3>Host app</h3>
        {this.renderHostInfo()}
...
export const GeneralContainer = connect<
  IGeneralContainerProps,
  IGeneralContainerDispatch,
  IOwn,
  IRootState
>(
  mapStateToProps,
