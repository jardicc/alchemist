...
export const all = (state: IRootState): ISorcererState =>
  state.inspector.sorcerer;
...
export const getActiveSnippet = createSelector(
  [all, getAllSnippets],
  (s, snippets) => {
    if (
      s.selectedItem.kind !== "snippet" ||
      typeof s.selectedItem.uuid !== "string"
    ) {
      return null;
    }
    const res =
      snippets.find((item) => item.$$$uuid === s.selectedItem.uuid) || null;
    return res;
  },
);
...
export const getActiveCommand = createSelector(
  [all, getAllEntryPoints],
  (s, entryPoints) => {
    if (
      s.selectedItem.kind !== "command" ||
      typeof s.selectedItem.uuid !== "string"
    ) {
      return null;
    }

    const res: IEntrypointCommand | null =
      (entryPoints.find(
        (item) =>
          item.$$$uuid === s.selectedItem.uuid && item.type === "command",
      ) as IEntrypointCommand) || null;
    return res;
  },
);
...
export const getActivePanel = createSelector(
  ...
  (s, entryPoints) => {
    ...
    const res =
      (entryPoints.find(
        (item) => item.$$$uuid === s.selectedItem.uuid && item.type === "panel",
      ) as IEntrypointPanel) || null;
    return res;
  },
);

export const getAllCommands = createSelector([getAllEntryPoints], (s) => {
  const res: IEntrypointCommand[] = s.filter(
    (item) => item.type === "command",
  ) as IEntrypointCommand[];
  return res;
});

export const getAllPanels = createSelector([getAllEntryPoints], (s) => {
  const res: IEntrypointPanel[] = s.filter(
...
export const getActiveItem = createSelector(
  ...
  (
    ...
    const res =
      ...
      activeCommand ||
      activePanel ||
      (genericModuleVisible ? { type: "general" } : null);
    return res;
  },
);

export const getManifestCode = createSelector(
  [all, getIndentString],
  (all, indent) => {
...
export const generateScriptFileCode = createSelector(
  [getAllCommands, getAllPanels, getAllSnippets],
  (commands, panels, snippets) => {
    const str = `const { entrypoints } = require("uxp");
...
export const generateHtmlFileCode = createSelector(
  [getAllPanels, getAllSnippets],
  (panels, snippets) => {
