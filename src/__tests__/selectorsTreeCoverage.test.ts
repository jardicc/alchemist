...
describe("inspectorContentSelectors", () => {
	test("trivial accessors: getInspectorContentTab/getContentPath/getContentActiveView/getSearchContentKeyword/getContentExpandedNodes/getContentExpandLevel", () => {
		const root = asRoot(reduceAll([
			setInspectorPathContentAction(["payload", "alpha"], "replace"),
			setInspectorViewAction("content", "raw"),
			setSearchContentKeywordAction("needle"),
	...
	test("getTreeContentUnfiltered – single selection follows treePath", () => {
		const root = asRoot(reduceAll([
			...
			setInspectorPathContentAction(["payload", "deep"], "replace"),
		]));
	...
	test("getTreeContentUnfiltered – primitive at path is wrapped in $$$noPin_<lastKey>", () => {
		const root = asRoot(reduceAll([
			...
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
			setInspectorPathContentAction(["payload", "beta"], "replace"),
		]));
	...
	test("getTreeContent – keyword matching null/undefined/number values", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			addDescriptorAction(desc("d1", {
				recordedData: {a: null, b: undefined, c: 42, nested: {x: 1}} as any,
			}), false),
			selectDescriptorAction("replace", "d1"),
			setSearchContentKeywordAction("null"),
		]));
		const r = Content.getTreeContent(root) as any;
		expect(r?.a).toBeNull();
	});
...
describe("inspectorDiffSelectors", () => {
	test("trivial accessors: getInspectorDifferenceTab/getDiffPath/getDiffActiveView/getDiffExpandedNodes/getDiffExpandLevel", () => {
		const root = asRoot(reduceAll([
			...
			setInspectorPathDiffAction(["payload", "alpha"], "replace"),
			setInspectorViewAction("diff", "raw"),
			setAutoExpandLevelAction("diff", 2),
		]));
	...
	test("getLeftTreeDiff / getRightTreeDiff – follow path on selected & secondary", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			addDescriptorAction(desc("d1", {recordedData: {payload: {leftVal: "L"}} as any}), false),
			addDescriptorAction(desc("d2", {recordedData: {payload: {rightVal: "R"}} as any}), false),
			selectDescriptorAction("replace", "d1"),
			selectDescriptorAction("add", "d2"),
			setInspectorPathDiffAction(["payload"], "replace"),
		]));
	...
	test("getLeftTreeDiff – falls back to autoActive when nothing is selected", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			addDescriptorAction(desc("d1", {recordedData: {a: 1} as any}), false),
		]));
	...
	test("getRightTreeDiff – falls back to secondaryAutoActive when nothing is selected (difference mode)", () => {
		const root = asRoot(reduceAll([
			...
			setModeTabAction("difference"),
			addDescriptorAction(desc("d1", {recordedData: {a: 1} as any}), false),
			addDescriptorAction(desc("d2", {recordedData: {a: 2} as any}), false),
		]));
	...
	test("getLeftRawDiff / getRightRawDiff cover the same paths", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			addDescriptorAction(desc("d1", {recordedData: {payload: {x: "L"}} as any}), false),
			addDescriptorAction(desc("d2", {recordedData: {payload: {x: "R"}} as any}), false),
			selectDescriptorAction("replace", "d1"),
			selectDescriptorAction("add", "d2"),
			setInspectorPathDiffAction(["payload", "x"], "replace"),
		]));
...
describe("inspectorDOMSelectors", () => {
	test("trivial accessors", () => {
		const root = asRoot(reduceAll([
			...
			setInspectorPathDomAction(["children", 0], "replace"),
			setAutoExpandLevelAction("DOM", 5),
		]));
	...
	test("getTreeDom – returns the descriptor's _target ref", () => {
		const root = asRoot(reduceAll([
			...
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
			setInspectorPathDomAction(["bounds"], "replace"),
		]));
	...
	test("getTreeDomInstance – walks the path on the mocked DOM result", () => {
		const root = asRoot(reduceAll([
			...
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
			setInspectorPathDomAction(["bounds", "right"], "replace"),
		]));
...
describe("inspectorCodeSelectors", () => {
	...
	test("getIndentString – 'tab' yields a tab, 'spaceN' yields N spaces", () => {
		const tabRoot = asRoot(reduceAll([setSettingsAction({indent: "tab"} as any)]));
		expect(Code.getIndentString(tabRoot)).toBe("\t");

		const sp4Root = asRoot(reduceAll([setSettingsAction({indent: "space4"} as any)]));
		expect(Code.getIndentString(sp4Root)).toBe("    ");

		const sp2Root = asRoot(reduceAll([setSettingsAction({indent: "space2"} as any)]));
	...
	test("getDescriptorOptions – multiple selected returns 'mixed' or the common value", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			addDescriptorAction(desc("d1", {descriptorSettings: {
				supportRawDataType: "auto",
				dialogOptions: "dontDisplay",
				modalBehavior: "execute",
				synchronousExecution: false,
			} as any}), false),
			addDescriptorAction(desc("d2", {descriptorSettings: {
	...
	test("getGeneratedCode – 'modal' wrapper renders both batchPlay + executeAsModal", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			setSettingsAction({codeImports: "require", codeWrappers: "modal", indent: "space2"} as any),
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
		]));
	...
	test("getGeneratedCode – 'batchPlay' wrapper omits executeAsModal", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			setSettingsAction({codeImports: "require", codeWrappers: "batchPlay"} as any),
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
		]));
		const code = Code.getGeneratedCode(root);
	...
	test("getGeneratedCode – 'array' wrapper returns just the descriptor array", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			setSettingsAction({codeImports: "import", codeWrappers: "array"} as any),
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
		]));
		const code = Code.getGeneratedCode(root);
	...
	test("getGeneratedCode – 'objects' wrapper trims the outer array brackets", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			setSettingsAction({codeImports: "import", codeWrappers: "objects"} as any),
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
		]));
		const code = Code.getGeneratedCode(root);
	...
	test("getGeneratedCode – notifier descriptor emits a warning banner", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			setSettingsAction({codeWrappers: "batchPlay"} as any),
			addDescriptorAction(desc("d1", {originalReference: {type: "notifier"} as any}), false),
			selectDescriptorAction("replace", "d1"),
		]));
	...
	test("getGeneratedCode – singleQuotes flag replaces double quotes", () => {
		const root = asRoot(reduceAll([
			setFilterStateAction("layer", "main", "off"),
			setSettingsAction({codeWrappers: "batchPlay", singleQuotes: true} as any),
			addDescriptorAction(desc("d1"), false),
			selectDescriptorAction("replace", "d1"),
		]));
		const code = Code.getGeneratedCode(root);
