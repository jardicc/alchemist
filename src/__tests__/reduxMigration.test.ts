...
describe("action creators – shape", () => {
	test("setModeTab returns a plain action with payload", () => {
		const a = setModeTab("content");
		expect(a).toEqual(expect.objectContaining({payload: "content"}));
		expect(typeof a.type).toBe("string");
	});

	test("setFontSize wraps payload", () => {
		const a = setFontSize("size-big");
		expect(a.payload).toBe("size-big");
		expect(typeof a.type).toBe("string");
	});

	test("addDescriptor nests {arg, replace}", () => {
		const desc = makeDescriptor("x");
		const a = addDescriptor(desc, true);
		expect(a.payload).toEqual({arg: desc, replace: true});
	});

	test("setData (atn) wraps array payload", () => {
		const sets = [makeAtnSet("a"), makeAtnSet("b")];
		const a = setData(sets);
		expect(a.payload).toEqual(sets);
	});

	test("make carries {type}", () => {
		const a = make("snippet");
...
describe("inspector reducer – behavior", () => {
	...
	test("SET_MODE_TAB updates inspector.activeTab", () => {
		...
		const s1 = inspectorReducer(s0, setModeTab("difference"));
	...
	test("TOGGLE_SETTINGS flips settings.settingsVisible", () => {
		...
		const s1 = inspectorReducer(s0, toggleSettings());
		const s2 = inspectorReducer(s1, toggleSettings());
	...
	test("SET_FONT_SIZE writes settings.fontSize", () => {
		const s = inspectorReducer(getInitialState(), setFontSize("size-big"));
		expect(s.settings.fontSize).toBe("size-big");
	});

	test("SET_SELECTED_REFERENCE_TYPE_ACTION updates top-level field", () => {
		const s = inspectorReducer(getInitialState(), setSelectedReferenceType("document"));
	...
	test("listener / spy / auto-inspector are mutually exclusive when enabled", () => {
		...
		s = inspectorReducer(s, setListener(true));
...
describe("atn reducer (via root) – behavior", () => {
	test("SET_DATA appends, CLEAR_ALL empties", () => {
		let s = getInitialState();
		s = inspectorReducer(s, setData([makeAtnSet("a"), makeAtnSet("b")]));
	...
	test("SET_DONT_SEND_DISABLED writes flag", () => {
		const s = inspectorReducer(getInitialState(), setDontSendDisabled(true));
		expect(s.atnConverter.dontSendDisabled).toBe(true);
	});

	test("SELECT_ACTION (replace) stores selection", () => {
		let s = getInitialState();
		s = inspectorReducer(s, setData([makeAtnSet("a")]));
		s = inspectorReducer(s, selectAction("replace", ["a"]));
...
describe("sorcerer reducer (via root) – behavior", () => {
	test("SELECT records the selected item kind/uuid", () => {
		const s = inspectorReducer(getInitialState(), select("snippet", "uuid-1"));
	...
	test("MAKE snippet appends to snippets.list", () => {
		...
		const s1 = inspectorReducer(s0, make("snippet"));
	...
	test("MAKE panel/command appends to manifest entrypoints", () => {
		...
		const s1 = inspectorReducer(s0, make("panel"));
		const s2 = inspectorReducer(s1, make("command"));
		expect(s2.sorcerer.manifestInfo.entrypoints.length).toBe(before + 2);
	});

	test("SET_MAIN merges into manifestInfo", () => {
		const s = inspectorReducer(getInitialState(), setMain({name: "MyPlugin", version: "9.9.9"}));
	...
	test("SET_SNIPPET updates a snippet by uuid", () => {
		...
		const s1 = inspectorReducer(
			s0,
			setSnippet({code: "// new"}, firstSnippetUuid),
		);
...
describe("full store – end-to-end sequence", () => {
	test("dispatching a sequence produces a stable state snapshot", () => {
		...
		store.dispatch(setModeTab("difference"));
		store.dispatch(setFontSize("size-big"));
		store.dispatch(setSelectedReferenceType("document"));
		store.dispatch(addDescriptor(makeDescriptor("d1"), false));
		store.dispatch(addDescriptor(makeDescriptor("d2"), false));
		store.dispatch(selectDescriptor("replace", "d1"));
		store.dispatch(lockDesc(true, ["d2"]));
		store.dispatch(setListener(true));
		store.dispatch(setData([makeAtnSet("set-a")]));
		store.dispatch(expandAction(["set-a"], true, false));
		store.dispatch(make("snippet"));
		store.dispatch(setMain({name: "SnapshotPlugin"}));
