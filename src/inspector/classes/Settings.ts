...
export class Settings {
  ...
  public static async exportDescriptorItems(
    ...
    const file = await localFileSystem.getFileForSaving(
      "Descriptor items " + Settings.getTimeStamp() + ".json",
      {
        types: ["json"],
      },
    );
  ...
  public static async saveSettingsWithDialog(object: any): Promise<void> {
    const file = await localFileSystem.getFileForSaving(
      "Alchemist state " + Settings.getTimeStamp() + ".json",
      {
        types: ["json"],
      },
    );
  ...
  public static importState(): IInspectorState | null {
    try {
      const text = readFileSync(`plugin-data:/${Settings.settingsFilename}`, {
        encoding: "utf-8",
      });
      ...
      console.log(
        "Error - with reading of settings! If you run this plugin for first time then settings file does not exist and that is fine. It will be created automatically later.",
      );
      // console.error(e);
  ...
  public static setSpectrumComponentSize(fontSize: TFontSizeSettings): void {
    switch (fontSize) {
      case "size-tiny":
        SpectrumComponetDefaults.defaultSize = "s";
        break;
      case "size-small":
        SpectrumComponetDefaults.defaultSize = "s";
        break;
      case "size-default":
        ...
        break;
      case "size-bigger":
        SpectrumComponetDefaults.defaultSize = "m";
        break;
      case "size-big":
        SpectrumComponetDefaults.defaultSize = "m";
        break;
      case "size-youMustBeJoking":
        SpectrumComponetDefaults.defaultSize = "xl";
        break;
    }
  }
