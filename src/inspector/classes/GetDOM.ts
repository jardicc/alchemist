...
export class ReferenceToDOM extends Reference {
  ...
  private getHistoryDom() {
    ...
    const history = new (doc.activeHistoryState as any).constructor(
      historyItem._id,
      doc.id,
    );
    return history;
  ...
  private getChannelDom() {
    ...
    if ("_enum" in this.channel) {
      const compositeChannel = new (
        doc.compositeChannels[0] as any
      ).constructor(doc.id, this.channel._value);
