...
export const guessOriginalReference = (
  refAM: TReference[],
): TAllTargetReferences | null => {
  ...
  switch (r.targetClass) {
    ...
    case "channel": {
      ...
      const res: IRefChannel = {
        ...
        channelID:
          channel && "_enum" in channel
            ? (channel._value as TChannelReferenceValid)
            : channel?._id ?? (init.channel.layerID as TChannelReferenceValid),
      };
      return res;
    }
    default: {
