...
export class IDBySelected {
  ...
  public static async channel(
    docID?: number,
  ): Promise<number | null | TChannelReferenceValid> {
    const resultItemIndex = await this.getProperty(
      "itemIndex",
      "channel",
      docID,
    );
    ...
    const itemIndex = (await this.getProperty("itemIndex", "channel", docID))
      .itemIndex;
  ...
  public static async path(
    docID?: number,
  ): Promise<"vectorMask" | "workPath" | number | null> {
    const pathKind = (await this.getProperty("kind", "path", docID)).kind
      ._value;
  ...
  public static async getProperty(
    ...
    const result = await action.batchPlay([desc], {});
    return result[0];
  }
}
