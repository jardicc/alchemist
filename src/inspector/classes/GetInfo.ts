...
export class GetInfo {
  ...
  private static async getReference(
    _originalRef: TAllTargetReferences,
    ...
    switch (origRef.type) {
      ...
      case "path": {
        if (
          origRef.type === "channel" &&
          !(origRef.channelID === "filterMask" || origRef.channelID === "mask")
        ) {
          break;
        }
    ...
    switch (origRef.type) {
      case "actions": {
        ...
        if (origRef.commandIndex !== "none" && origRef.actionID !== "none") {
          commandIndex = this.getActionCommandIndexByID(
            origRef.commandIndex,
            origRef.actionID,
          );
        }
      ...
      case "channel": {
        ...
        if (!ref.document) {
          throw new Error("It should have document reference");
        }
  ...
  public static async getAM(
    _originalRef: TAllTargetReferences | null,
  ): Promise<IDescriptor | null> {
    if (!_originalRef) {
      return null;
    }
    const origRef = cloneDeep(_originalRef);
    ...
    if (reference.propertiesOnly.length > 1 || reference.allClass) {
      ...
      if (reference.allClass === "layer") {
        if (reference.document) {
          const hasBg = new app.Document(reference.document._id)
            .backgroundLayer;
          if (hasBg) {
            range.index = 0;
          }
  ...
  public static generateTitle = (
    originalReference: TAllTargetReferences | null,
    ...
    switch (originalReference.type) {
      ...
      case "listener": {
        ...
        const target: any =
          calculatedReference._target || calculatedReference.null;
        ...
        const res =
          calculatedReference._obj +
          (postfix.trim() ? " (" + postfix.trim() + ")" : "");
        return res;
      }
      case "notifier": {
  ...
  private static buildReply(
    startTime: number,
    playResult: ActionDescriptor[],
    playAbleData: ITargetReferenceAM,
    originalRef: TAllTargetReferences,
  ): IDescriptor {
    return {
      startTime,
      endTime: Date.now(),
  ...
  private static async getFromGenerator(
    originalRef: TAllTargetReferences,
  ): Promise<any> {
    const startTime = Date.now();
  ...
  private static getActionCommandIndexByID(
    commandID: number,
    actionItemID: number,
  ): number {
