...
export const getSelectedTargetReference = createSelector(
  [all],
  (s) => s.selectedReferenceType,
);
export const getFilterBySelectedReferenceType = createSelector(
  [all],
  (s) => s.filterBySelectedReferenceType,
);
export const getTargetReference = createSelector(
  [all],
  (s) => s.targetReference,
);
export const getAutoUpdate = createSelector(
  [all],
  (s) => s.settings.autoUpdateInspector,
);
export const getAllDescriptors = createSelector([all], (s) => s.descriptors);
export const getSelectedDescriptors = createSelector([all], (s) =>
  s.descriptors.filter((d) => d.selected === true),
);
export const getSelectedDescriptorsUUID = createSelector(
  [getSelectedDescriptors],
  (s) => s.map((d) => d.id),
);
export const getPropertySettings = createSelector(
  [all],
  (s) => s.settings.properties,
);
export const getLockedSelection = createSelector(
  [getSelectedDescriptors],
  (s) => s.some((d) => d.locked),
);
export const getPinnedSelection = createSelector(
  [getSelectedDescriptors],
  (s) => s.some((d) => d.pinned),
);
export const getRemovableSelection = createSelector(
  [getLockedSelection],
  (s) => !s,
);

export const getActiveRef = createSelector(
  [getTargetReference, getSelectedTargetReference],
  (targets, selected) => {
    switch (selected) {
      case "listener":
        return targets.listener;
      case "dispatcher":
        return targets.dispatcher;
      case "notifier":
        return targets.notifier;
      case "replies":
        return targets.replies;
      case "generator":
        return targets.generator;
      case "application":
        return targets.application;
      case "document":
        return targets.document;
      case "layer":
        return targets.layer;
      case "path":
        return targets.path;
      case "channel":
      ...
      case "animationFrameClass":
        return targets.animationFrameClass; //
      case "animationClass":
        return targets.animationClass; //
      case "historyState":
        return targets.historyState; //
      case "snapshotClass":
        return targets.snapshotClass; //
      case "guide":
        return targets.guide;
      default: {
...
export const getCategoryItemsVisibility = createSelector(
  [all, getActiveRef],
  (s, activeRef) => {
    // adds currently selected category into visibility even though is not explicitly set to visible
    const res: TTargetReference[] = [
      ...new Set([...s.explicitlyVisibleTopCategories, activeRef.type]),
    ];
    return res;
  },
);
...
export const getFontSizeSettings = createSelector(
...
export const getSettingsVisible = createSelector(
  [getInspectorSettings],
  (s) => {
...
export const getLeftColumnWidth = createSelector(
  [getInspectorSettings],
  (s) => {
...
export const getRightColumnWidth = createSelector(
  [getInspectorSettings],
  (s) => {
...
export const getPropertiesListForActiveRef = createSelector(
  [getPropertySettings, getActiveRef],
  (propertySettings, activeRef) => {
    const foundSettings = propertySettings.find(
      (p) => p.type === activeRef.type,
    );
    return foundSettings;
  },
);

export const getNeverRecordActionNames = createSelector(
  [getInspectorSettings],
  (s) => s.neverRecordActionNames,
);
...
export const getListenerNotifierFilterSettings = createSelector(
  [all],
  (all) => {
...
export const getDescriptorsListView = createSelector(
  ...
  (allDesc, activeRef, rootFilter, settings, visibleMainCategories) => {
	...
	*/
    ...
    if (activeRef.type === "listener" || activeRef.type === "notifier") {
      const filterSettings: IListenerNotifierFilter =
        activeRef.type === "listener" ? listenerFilter : notifierFilter;
      if (
        filterSettings.type === "exclude" &&
        filterSettings.exclude.join(";").trim().length
      ) {
        filtered = filtered.filter(
          (item) =>
            !filterSettings.exclude.some(
              (str) =>
                (item.recordedData as ActionDescriptor)?._obj?.includes(
                  str.trim(),
                ),
            ),
        );
      } else if (
        filterSettings.type === "include" &&
        filterSettings.include.join(";").trim().length
      ) {
        filtered = filtered.filter((item) =>
          filterSettings.include.some(
            (str) =>
              (item.recordedData as ActionDescriptor)?._obj?.includes(
                str.trim(),
              ),
      ...
      for (let i = 0; i < filtered.length - 1; i++) {
        ...
        for (let j = i + 1; j < filtered.length; j++) {
          ...
          if (next.crc === element.crc) {
            filtered.splice(j, 1);
            j -= 1;
            element.groupCount = !element.groupCount
              ? 2
              : element.groupCount + 1;
          }
        }
      }
...
export const getAutoActiveDescriptor = createSelector(
  [getActiveDescriptors, getDescriptorsListView],
  (activeDescs, view) => {
...
export const getSecondaryAutoActiveDescriptor = createSelector(
  [getActiveDescriptors, getDescriptorsListView],
  (activeDescs, view) => {
    const list = view; //.filter(item => !item.pinned);
    if (activeDescs.length === 0) {
...
export const getAutoSelectedUUIDs = createSelector(
  [getAutoActiveDescriptor, getSecondaryAutoActiveDescriptor, getModeTabID],
  (first, second, mode) => {
    const f = first?.id;
...
export const getHasAutoActiveDescriptor = createSelector(
...
export const getActiveDescriptorOriginalReference = createSelector(
  [getActiveDescriptors, getAutoActiveDescriptor],
  (selected, autoActive) => {
...
export const getReplayEnabled = createSelector(
  ...
  (selected) => {
    ...
    if (
      selected.some((item) =>
        ["replies", "dispatcher"].includes(item.originalReference.type),
      )
    ) {
      return false;
    }
...
export const getCopyToClipboardEnabled = createSelector(
  [getReplayEnabled],
  (enabled) => {
    return enabled;
  },
...
export const getRanameEnabled = createSelector(
  [getDescriptorsListView],
  (all) => {
...
export const getAddAllowed = createSelector([getActiveRef], (activeRef) => {
  ...
  if (
    (
      ["listener", "dispatcher", "notifier", "replies"] as TTargetReference[]
    ).includes(activeRef.type)
  ) {
    return false;
  }
