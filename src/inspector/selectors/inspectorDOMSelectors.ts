...
export const getTreeDom = createSelector(
  [
    getSelectedDescriptors,
    getDomPath,
    getSelectedTargetReference,
  ...
  (selectedDesc, domPath, mainClass, autoSelectedDesc) => {
    if (
      (!selectedDesc.length && !autoSelectedDesc) ||
      mainClass === "listener"
    ) {
      return {
        ref: null,
    ...
    const ref = {
      ...
      ref:
        (selectedDesc?.[0]?.playAbleData as ITargetReferenceAM)?._target ??
        (autoSelectedDesc?.playAbleData as ITargetReferenceAM)?._target,
      path: domPath,
    };
    return ref;
  },
