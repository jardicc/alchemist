...
type BatchPlayCommandOptionsExtended = BatchPlayCommandOptions & {
  synchronousExecution?: boolean;
};

export const getDescriptorOptions = createSelector(
  [getActiveDescriptors, getAutoActiveDescriptor, getInspectorSettings],
  (selected, autoActive, settings) => {
    ...
    const res: IDescriptorSettings = {
      supportRawDataType: getValue(
        desc.map((item) => item.descriptorSettings.supportRawDataType),
      ),
      dialogOptions: getValue(
        desc.map((item) => item.descriptorSettings.dialogOptions),
      ),
      modalBehavior: getValue(
        desc.map((item) => item.descriptorSettings.modalBehavior),
      ),
      synchronousExecution: getValue(
        desc.map((item) => item.descriptorSettings.synchronousExecution),
      ),
    };
...
export const getIndentString = createSelector(
  [getInspectorSettings],
  (settings) => {
    let indent = "\t";
    ...
    if (settings.indent !== "tab") {
      indent = " ".repeat(
        parseInt(settings.indent.charAt(settings.indent.length - 1)),
      );
    }
...
export const getGeneratedCode = createSelector(
  ...
  (
    ...
    function addCommonOptions(
      data: IDescriptor[],
    ): BatchPlayCommandOptionsExtended {
      const hasAnyAsync = data.some(
        (item) => item.descriptorSettings.synchronousExecution === false,
      );
      const hasAnySync = data.some(
        (item) => item.descriptorSettings.synchronousExecution === true,
      );

      const modalIsExecute = data.every(
        (item) => item.descriptorSettings.modalBehavior === "execute",
      );
      const modalIsWait = data.every(
        (item) => item.descriptorSettings.modalBehavior === "wait",
      );
      const modalIsDefault = data.every(
        (item) => !item.descriptorSettings.modalBehavior,
      );

      const res: BatchPlayCommandOptionsExtended = {};

      if (hasAnySync) {
        res.synchronousExecution = true;
      } else if (hasAnyAsync) {
        res.synchronousExecution = false;
      }

      if (modalIsExecute) {
        res.modalBehavior = "execute";
      } else if (modalIsWait) {
        res.modalBehavior = "wait";
      } else if (!modalIsDefault) {
        res.modalBehavior = "fail";
      }
    ...
    if (selected.length >= 1 || autoActive) {
      ...
      const playAbleInfo =
        "// Alchemist can't generate code from the reply of replay and from dispatched code.";
      ...
      const strExecModalImport = addModules
        ? qts(`const {executeAsModal} = require("photoshop").core;\n`)
        : "";
      const strBatchPlayImport = addModules
        ? qts(`const {batchPlay} = require("photoshop").action;\n\n`)
        : "";
      ...
      if (wrappers === "batchPlay") {
        return banner + strBatchPlayImport + strBatchPlay;
        ...
        return banner + strDesc;
      } else if (wrappers === "objects") {
        return banner + udt(strDesc.slice(2, -2));
      }
...
export const getGeneratedCode2 = createSelector(
  ...
  (
    ...
    if (selected.length >= 1 || autoActive) {
      ...
      if (
        iDesc.some((item) =>
          ["replies", "notifier", "dispatcher"].includes(
            item.originalReference.type,
          ),
        )
      ) {
