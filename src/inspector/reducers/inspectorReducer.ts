...
export const inspectorReducer = (
  ...
  switch (action.type) {
      ...
      state = produce(state, (draft) => {
        ...
        if (found && operation !== "none") {
          if (operation === "add" || operation === "replace") {
            ...
            const lastSelectedItemIndex = view
              .map((item) => item.id)
              .indexOf(state.settings.lastSelectedItem ?? "n/a");
            const thisItemIndex = view
              .map((item) => item.id)
              .indexOf(uuid as string);
            if (lastSelectedItemIndex !== -1 && thisItemIndex !== -1) {
              const ids: string[] = [];
              for (
                let i = Math.min(lastSelectedItemIndex, thisItemIndex),
                  end = Math.max(lastSelectedItemIndex, thisItemIndex);
                i <= end;
                i++
              ) {
                ids.push(view[i].id);
              ...
              ids.forEach((id) => {
                ...
                if (f) {
                  f.selected = operation === "addContinuous";
                }
              });
            }
          }
        ...
        //
        draft.settings.lastSelectedItem =
    ...
    case "LOCK_DESC": {
      state = produce(state, (draft) => {
        if (state.settings.groupDescriptors === "strict") {
          const selectedByID = state.descriptors.filter((d) =>
            action.payload.uuids.includes(d.id),
          );
          const crcs = Array.from(new Set(selectedByID.map((d) => d.crc)));

          draft.descriptors
            .filter((d) => crcs.includes(d.crc))
            .forEach((d) => (d.locked = action.payload.lock));
        } else if (state.settings.groupDescriptors === "none") {
          draft.descriptors
            .filter((d) => action.payload.uuids.includes(d.id))
            .forEach((d) => (d.locked = action.payload.lock));
        }
      });
      break;
    ...
    case "PIN_DESC": {
      state = produce(state, (draft) => {
        if (state.settings.groupDescriptors === "strict") {
          ...
          draft.descriptors
            .filter((d) => crcs.includes(d.crc))
            .forEach((d) => (d.pinned = action.payload.pin));
        } else if (state.settings.groupDescriptors === "none") {
          draft.descriptors
            .filter((d) => action.payload.uuids.includes(d.id))
            .forEach((d) => (d.pinned = action.payload.pin));
    ...
    case "REMOVE_DESC": {
      state = produce(state, (draft) => {
        if (state.settings.groupDescriptors === "strict") {
          const selectedByID = state.descriptors.filter(
            (d) => action.payload.includes(d.id) && !d.locked,
          );
          // remove by crc instead of ID.
          const crcs = Array.from(new Set(selectedByID.map((d) => d.crc)));
          draft.descriptors = state.descriptors.filter(
            (d) => crcs.includes(d.crc) === false || d.locked,
          );
        } else if (state.settings.groupDescriptors === "none") {
          draft.descriptors = state.descriptors.filter(
            (d) => action.payload.includes(d.id) === false || d.locked,
          );
        }
      });
      break;
    ...
    case "SET_INSPECTOR_PATH_DIFF": {
      state = produce(state, (draft) => {
        if (action.payload.mode === "add") {
          draft.inspector.difference.treePath = [
    ...
    case "SET_INSPECTOR_PATH_CONTENT": {
      state = produce(state, (draft) => {
        if (action.payload.mode === "add") {
          draft.inspector.content.treePath = [
            ...state.inspector.content.treePath,
            ...action.payload.path,
          ];
        } else if (action.payload.mode === "replace") {
    ...
    case "SET_INSPECTOR_PATH_DOM": {
      state = produce(state, (draft) => {
        if (action.payload.mode === "add") {
          draft.inspector.dom.treePath = [
            ...state.inspector.dom.treePath,
            ...action.payload.path,
          ];
        } else if (action.payload.mode === "replace") {
      ...
      state = produce(state, (draft) => {
        if (action.payload.kind === "append") {
          action.payload.items.forEach(
            (desc) => (desc.id = crypto.randomUUID()),
          );
    ...
    case "SET_FILTER_STATE": {
      state = produce(state, (draft) => {
        const {
          payload: { state: filterState, subType, type },
        } = action;
    ...
    case "SET_EXPANDED_PATH": {
      state = produce(state, (draft) => {
        ...
        switch (type) {
          case "content":
            draftPart = draft.inspector.content;
            break;
          case "difference":
            draftPart = draft.inspector.difference;
            break;
          case "dom":
            draftPart = draft.inspector.dom;
            break;
          default:
            console.warn("You shouldn't see this line logged in console");
        }
    ...
    case "RENAME_DESCRIPTOR": {
      state = produce(state, (draft) => {
        const found = draft.descriptors.find(
          (desc) => desc.id === action.payload.uuid,
        );
        if (found) {
    ...
    case "TOGGLE_DESCRIPTORS_GROUPING": {
      state = produce(state, (draft) => {
        if (action.payload === null) {
          draft.settings.groupDescriptors =
            state.settings.groupDescriptors === "strict" ? "none" : "strict";
        } else {
    ...
    case "TOGGLE_ACCORDION": {
      state = produce(state, (draft) => {
        if (action.payload.expanded) {
          if (
            !state.settings.accordionExpandedIDs.includes(action.payload.id)
          ) {
          ...
          if (state.settings.accordionExpandedIDs.includes(action.payload.id)) {
            draft.settings.accordionExpandedIDs =
              state.settings.accordionExpandedIDs.filter(
