...
import {
  ...
  getRightTreeDiff,
  getDiffPath,
  getDiffExpandedNodes,
  getLeftRawDiff,
  getRightRawDiff,
  getDiffActiveView,
  getDiffExpandLevel,
} from "../../selectors/inspectorDiffSelectors";
...
> = (dispatch: Dispatch): ITreeDiffDispatch => ({
  onInspectPath: (path, mode) =>
    dispatch(setInspectorPathDiffAction(path, mode)),
  onSetExpandedPath: (path, expand, recursive, data) =>
    dispatch(
      setExpandedPathAction("difference", path, expand, recursive, data),
    ),
  onSetView: (viewType) => dispatch(setInspectorViewAction("diff", viewType)),
  onSetAutoExpandLevel: (level) =>
    dispatch(setAutoExpandLevelAction("diff", level)),
});

export const TreeDiffContainer = connect<
  ITreeDiffProps,
  ITreeDiffDispatch,
  Record<string, unknown>,
  IRootState
