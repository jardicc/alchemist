...
export class AccDrop extends React.Component<TAccDrop, IAccDropState> {
  ...
  private getLabel = () => {
    ...
    const labels = newList
      .filter((item) => this.props.selected.includes(item.value))
      .map((item) => item.label);
  ...
  private renderHeader = (): JSX.Element => {
    ...
    return (
      <div
        key={"h_" + id}
        className={"AccDrop header " + (className || "")}
        onClick={void this.headerClick}
      >
  ...
  private renderGroup = (group: IPropertyGroup) => {
    return (
      <React.Fragment key={"f_" + group.group}>
        <div className="groupHeader" key={"g_" + group.group}>
          {group.groupLabel}
        </div>
  ...
  private renderItem = (item: IPropertyItem) => {
    ...
    if (
      ...
      return (
        ...
        >
          <div className="label">
            {icons && <div className="icon">{getIcon(item.value as any)}</div>}{" "}
            <span>{item.label}</span>
          </div>
          {
            // filter within main category dropdown
            ItemPostFix && (
              <div className="itemPostFix">
                <ItemPostFix value={item.value.toString()} />
              </div>
            )
          }
        </div>
  ...
  private renderContent = (): React.ReactNode => {
    ...
    return (
      <div
        key={"c_" + id}
        className={"AccDrop container " + (this.props.className || "")}
        style={containerMaxHeight}
      >
  ...
  componentDidUpdate(
    prevProps: Readonly<TAccDrop>,
    prevState: Readonly<IAccDropState>,
    snapshot?: any,
  ): void {
    if (this.heightRef) {
