...
export class ButtonMenu extends React.Component<TButtonMenu, IButtonMenuState> {
  ...
  private panelElement = () => {
    ...
    if (list) {
      return (
        document.body.querySelector("[panelid=inspector]") ?? document.body
      );
    }
    return null;
  };
  ...
  public componentDidMount(): void {
    ...
    const elContent = document
      .getElementById(this.state.uuid)
      ?.querySelector(".childWrap");
  ...
  public componentDidUpdate(
    prevProps: TButtonMenu,
    prevState: IButtonMenuState,
  ): void {
  ...
  private generatePosition = () => {
    ...
    switch (placementVertical) {
      case "bottom":
        result = { bottom: `-${this.state.listHeight}px` };
        break;
      case "top":
        result = { top: `-${this.state.listHeight}px` };
        break;
    }
  ...
  public render(): JSX.Element {
    ...
    return (
      <div
        id={this.state.uuid}
        className={"ButtonMenu" + " " + (this.props.className || "")}
      >
        <div
          className="childWrap"
          onClick={this.toggle}
