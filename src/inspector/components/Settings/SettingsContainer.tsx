...
class Settings extends Component<TSettings, ISettingsState> {
  ...
  private common = (options: Partial<IDescriptorSettings>) => {
    const {
      autoSelectedUUIDs,
      selected,
      onSetDescriptorOptions: onSetOptions,
    } = this.props;
  ...
  private renderOptionsScope = (): React.ReactNode => {
    ...
    if (auto?.length) {
      ...
      return (
        <>Will change {this.props.selected?.length ?? 0} selected item(s)</>
      );
    }
  };
  ...
  private onSetImports = (e: any) => {
    this.props.onSetGlobalOptions({
      codeImports: e.target?.checked ? "require" : "none",
    });
  };
  ...
  public render(): React.ReactNode {
    const {
      settings: {
        makeRawDataEasyToInspect: ignoreRawData,
        maximumItems,
        fontSize,
      ...
      onSetRecordRaw,
      onSetFontSize,
      onNeverRecordActionNamesChanged,
    } = this.props;
    const {
      onSetGlobalOptions,
    ...
    const btnSettings = (
      <div
        className={"FilterButton settings semi"}
        title="Show settings"
        onClick={setToggleSettings}
        style={{ position: "fixed", right: "1.5em", top: 0 }}
      >
    ...
    return (
      <div className="Settings">
        ...
        <Accordion
          id="batchPlaySettings"
          expanded={accordionExpandedIDs}
        ...
        >
          <div className="column">
              ...
              <SP.Dropdown quiet={false} className="fullW">
                <SP.Menu slot="options" onChange={this.onSynchronousExecution}>
                  <SP.MenuItem
                    key={"true"}
                    value={"true"}
                    selected={synchronousExecution === true ? true : undefined}
                  >
                    true
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"false"}
                    value={"false"}
                    selected={synchronousExecution === false ? true : undefined}
                  >
                    false
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"default"}
                    value={"default"}
                    selected={synchronousExecution === null ? true : undefined}
                  >
                    Default
                  </SP.MenuItem>
                </SP.Menu>
              ...
              <SP.Dropdown quiet={false} className="fullW">
                <SP.Menu slot="options" onChange={this.onSetDialogOptions}>
                  <SP.MenuItem
                    key={"silent"}
                    value={"silent"}
                    selected={dialogOptions === "silent" ? true : undefined}
                  >
                    silent (DialogModes.NO)
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"dontDisplay"}
                    value={"dontDisplay"}
                    selected={
                      dialogOptions === "dontDisplay" ? true : undefined
                    }
                  >
                    dontDisplay (DialogModes.ERROR)
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"display"}
                    value={"display"}
                    selected={dialogOptions === "display" ? true : undefined}
                  >
                    display (DialogModes.ALL)
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"default"}
                    value={"default"}
                    selected={dialogOptions === null ? true : undefined}
                  >
                    Default
                  </SP.MenuItem>
                </SP.Menu>
              ...
              <SP.Dropdown quiet={false} className="fullW">
                <SP.Menu slot="options" onChange={this.onSetModalBehavior}>
                  <SP.MenuItem
                    key={"wait"}
                    value={"wait"}
                    selected={modalBehavior === "wait" ? true : undefined}
                  >
                    wait
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"execute"}
                    value={"execute"}
                    selected={modalBehavior === "execute" ? true : undefined}
                  >
                    execute
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"fail"}
                    value={"fail"}
                    selected={modalBehavior === "fail" ? true : undefined}
                  >
                    fail
                  </SP.MenuItem>
                  <SP.MenuItem
                    key={"default"}
                    value={"default"}
                    selected={modalBehavior === null ? true : undefined}
                  >
                    Default
                  </SP.MenuItem>
                </SP.Menu>
            ...
            <div className="row">
              <SP.Checkbox
                onChange={this.onSetSupportRawDataType}
                checked={!!supportRawDataType || undefined}
                indeterminate={
                  supportRawDataType === "mixed" ? true : undefined
                }
              >
                Generate raw data type into source code. (might slow down panel
                when turned on)
              </SP.Checkbox>
            </div>
          </div>
        ...
        >
          <span className="scope">
            Global options for all items including already recorded
          </span>
          <div className="column indent">
            <div className="label">Indent using:</div>
            <SP.Dropdown quiet={false} className="fullW">
              <SP.Menu
                slot="options"
                onChange={this.onSetIndent}
                className="fullW"
              >
                <SP.MenuItem
                  key={"tab"}
                  value={"tab"}
                  selected={indent === "tab" ? true : undefined}
                >
                  1 tab
                </SP.MenuItem>
                ...
                <SP.MenuItem
                  key={"space1"}
                  value={"space1"}
                  selected={indent === "space1" ? true : undefined}
                >
                  1 space
                </SP.MenuItem>
                <SP.MenuItem
                  key={"space2"}
                  value={"space2"}
                  selected={indent === "space2" ? true : undefined}
                >
                  2 spaces
                </SP.MenuItem>
                <SP.MenuItem
                  key={"space3"}
                  value={"space3"}
                  selected={indent === "space3" ? true : undefined}
                >
                  3 spaces
                </SP.MenuItem>
                <SP.MenuItem
                  key={"space4"}
                  value={"space4"}
                  selected={indent === "space4" ? true : undefined}
                >
                  4 spaces
                </SP.MenuItem>
                <SP.MenuItem
                  key={"space5"}
                  value={"space5"}
                  selected={indent === "space5" ? true : undefined}
                >
                  5 spaces
                </SP.MenuItem>
                <SP.MenuItem
                  key={"space6"}
                  value={"space6"}
                  selected={indent === "space6" ? true : undefined}
                >
                  6 spaces
                </SP.MenuItem>
                <SP.MenuItem
                  key={"space7"}
                  value={"space7"}
                  selected={indent === "space7" ? true : undefined}
                >
                  7 spaces
                </SP.MenuItem>
                <SP.MenuItem
                  key={"space8"}
                  value={"space8"}
                  selected={indent === "space8" ? true : undefined}
                >
                  8 spaces
                </SP.MenuItem>
              </SP.Menu>
          ...
          <div className="row">
            <SP.Checkbox
              onChange={(e) =>
                onSetGlobalOptions({ singleQuotes: !!e.target?.checked })
              }
          ...
          <div className="row">
            <SP.Checkbox
              onChange={(e) =>
                onSetGlobalOptions({ hide_isCommand: !!e.target?.checked })
              }
              checked={hide_isCommand || undefined}
            >
              Hide property &quot;_isCommand&quot;
            </SP.Checkbox>
          </div>
          <div className="row">
            <SP.Checkbox
              onChange={(e) =>
                onSetGlobalOptions({ hideDontRecord: !!e.target?.checked })
              }
              checked={hideDontRecord || undefined}
            >
              Hide property &quot;dontRecord&quot;
            </SP.Checkbox>
          </div>
          <div className="row">
            <SP.Checkbox
              onChange={(e) =>
                onSetGlobalOptions({ hideForceNotify: !!e.target?.checked })
              }
              checked={hideForceNotify || undefined}
            >
              Hide property &quot;forceNotify&quot;
            </SP.Checkbox>
          </div>
        </Accordion>
        ...
        >
          <div className="column">
            <span className="fontSizeLabel">Font size:</span>
            <SP.Dropdown className="fontSizeDropdown fullW">
              <SP.Menu
                slot="options"
                onChange={(e) =>
                  onSetFontSize(items[e.target?.selectedIndex ?? 0].val)
                }
              >
                {items.map((item) => (
                  <SP.MenuItem
        ...
        <Accordion
          ...
          expanded={accordionExpandedIDs}
          onChange={onToggleAccordion}
          header="Ignored Actions"
        >
          <div className="column">
            <div>
              <span>
                Events that never will be recorded. No matter what you will set
                in include/exclude filter. One per line, no quotes, no commas or
                semicolons. Will Not affect already recorded items.
              </span>
            </div>
            <div>
              <SP.Textarea
                ...
                onInput={(e: any) =>
                  onNeverRecordActionNamesChanged(e.currentTarget.value)
                }
        ...
        >
          <div className="row">
            <SP.Checkbox
              checked={ignoreRawData ? true : undefined}
              onChange={(e) => onSetRecordRaw(!!e.target?.checked)}
            >
              Record raw data type as an array of numbers to make it easily
              readable (might slow down Alchemist)
            </SP.Checkbox>
          </div>
...
const mapDispatchToProps = (dispatch: Dispatch): ISettingsDispatch => ({
  ...
  onNeverRecordActionNamesChanged: (value) =>
    dispatch(setNeverRecordActionNamesAction(value)),

  onSetGlobalOptions: (options) => dispatch(setSettingsAction(options)),
  onSetDescriptorOptions: (uuids, options) =>
    dispatch(setDescriptorOptionsAction(uuids, options)),
