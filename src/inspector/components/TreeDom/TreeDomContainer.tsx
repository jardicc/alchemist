...
import {
  ...
  getDomExpandedNodes,
  getDOMExpandLevel,
} from "../../selectors/inspectorDOMSelectors";
        ...
        <div className="TreeDomBox">
          {content === undefined || content === null ? (
            <div className="message">
              Content is missing. Please make sure that your selected descriptor
              and your pinned property exists
            </div>
          ) : (
            <JSONTree
...
> = (dispatch): ITreeDomDispatch => ({
  onInspectPath: (path, mode) =>
    dispatch(setInspectorPathDomAction(path, mode)),
  onSetExpandedPath: (path, expand, recursive, data) =>
    dispatch(setExpandedPathAction("dom", path, expand, recursive, data)),
  onSetAutoExpandLevel: (level) =>
    dispatch(setAutoExpandLevelAction("DOM", level)),
});

export const TreeDomContainer = connect<
  ITreeDomProps,
  ITreeDomDispatch,
  Record<string, unknown>,
  IRootState
