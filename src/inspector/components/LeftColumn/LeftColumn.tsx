...
import {
  ...
  getAutoUpdate,
  getAddAllowed,
  getSelectedDescriptorsUUID,
  getLockedSelection,
  getPinnedSelection,
  getRemovableSelection,
  getDescriptorsListView,
  getHasAutoActiveDescriptor,
...
export class LeftColumn extends React.Component<TLeftColumn, IState> {
  ...
  private getDescriptor = async (): Promise<void> => {
    ...
    if (result === null) {
      NotificationManager.error(
        "Please make sure that item you want to add exists in Photoshop",
        "Failed",
        3500,
      );
      return;
  ...
  public autoInspector = async (
    ...
    const playResult = await photoshop.action.batchPlay(
      [calculatedReference],
      {},
    );
  ...
  public listener = (event: string, descriptor: any, spy = false): void => {
    ...
    const result: IDescriptor = {
      ...
      title:
        (spy ? "[S] " : "") +
        GetInfo.generateTitle(originalReference, descWithEvent),
      descriptorSettings: this.props.settings.initialDescriptorSettings,
    };
  ...
  private attachListener = async () => {
    const {
      settings: { dontShowMarketplaceInfo, autoUpdateListener },
    } = this.props;
    if (
      !dontShowMarketplaceInfo &&
      !autoUpdateListener &&
      !Main.devMode &&
      !Main.isFirstParty
    ) {
  ...
  private renderDescriptorsList = (): React.ReactNode => {
    ...
    return allInViewDescriptors.map((d, index) => {
      return (
        <div
          className={"DescriptorItem"}
          key={index}
          ref={
            index === allInViewDescriptors.length - 1
              ? (this.lastDescRef as any)
              : null
          }
        >
  ...
  private onPlaySeparated = async () => {
    ...
    for await (const item of toPlay) {
      ...
      try {
        descriptors = await replayDescriptor(
          item.playAbleData as ActionDescriptor,
        );
      } catch (e: any) {
      ...
      const result: IDescriptor = {
        ...
        recordedData: descriptors
          ? RawDataConverter.replaceArrayBuffer(descriptors)
          : null,
        originalReference,
        pinned: false,
        selected: false,
        ...
        title: GetInfo.generateTitle(
          originalReference,
          item.playAbleData as ITargetReferenceAM,
        ),
        descriptorSettings: this.props.settings.initialDescriptorSettings,
      };
  ...
  private renderMarketplaceDialog = () => {
    ...
    return (
      <dialog className="MarketplaceAdvice" ref={this.marketplaceDialogRef}>
        <form>
          <sp-heading>This is marketplace version of Alchemist</sp-heading>
          <sp-body>
            <p>
              This version will not listen to all events due to limitation in
              Photoshop but it will try to listen as much as possible. If you
              know such a event please post it{" "}
              <a href="https://github.com/jardicc/alchemist/issues/3">here</a>.
            </p>
            <p>
              Also Alchemist might be unresponsive for several seconds once you
              click continue.{" "}
              <a href="https://github.com/jardicc/alchemist">
                Get development version here
              </a>
            </p>
          </sp-body>
          <footer>
            <label className="dontShowLabel">
              <SP.Checkbox
                onChange={(e: any) =>
                  onSetDontShowMarketplaceInfo(e.currentTarget.checked)
                }
              />
              {"Don't show again"}
            </label>
            ...
            <sp-button
              variant="cta"
              onClick={() => {
                this.marketplaceDialogRef.current.close("true");
              }}
            >
              Continue
            </sp-button>
  ...
  private copyToClipboard = () => {
    (navigator.clipboard as any).setContent({
      "text/plain": this.props.generatedCode,
    });
  };
      ...
      <div className="Filters LeftColumn">
        <div className="oneMore">
          ...
          <div className="search">
            <SP.Textfield
              placeholder="Search..."
              onInput={(e: any) => this.onSearch(e.currentTarget.value)}
              value={searchTerm || ""}
              quiet
            />
            <SP.Checkbox
              onChange={this.props.toggleDescGrouping}
              checked={groupDescriptors === "strict"}
            >
              {" "}
          ...
          <div
            className="descriptorsWrapper"
            ref={this.wrapperDescRef}
          ...
          <div className="descriptorButtons">
              ...
              items={
                <div className="column">
                  ...
                  <div
                    className="button"
                    onMouseDown={() => {
                      onClearNonExistent(filterNonExistent(allDescriptors));
                    }}
                  >
                    Non-existent
                  </div>
            ...
            <div
              ...
              className={
                "rename buttonIcon " +
                (renameEnabled ? "allowed" : "disallowed")
              }
              onClick={this.rename}
            >
              <IconPencil />
            ...
            <div
              ...
              className={
                "clipboard buttonIcon " +
                (copyToClipboardEnabled ? "allowed" : "disallowed")
              }
            ...
            <div
              title="Replay"
              className={
                "play buttonIcon " + (replayEnabled ? "" : "disallowed")
              }
              onClick={void this.onPlaySeparated}
            >
              <IconPlayIcon />
            </div>
            <div
              ...
              className={
                "lock buttonIcon " +
                (selectedDescriptors?.length ? "" : "disallowed")
              }
              onClick={() => {
                onLock(!lockedSelection, selectedDescriptorsUUIDs);
              }}
            >
              {selectedDescriptors.some((desc) => desc.locked) ? (
                <IconLockUnlocked />
              ) : (
                <IconLockLocked />
              )}
            ...
            <div
              ...
              onClick={() => {
                onPin(!pinnedSelection, selectedDescriptorsUUIDs);
              }}
            >
              {selectedDescriptors.some((desc) => desc.pinned) ? (
                <IconPinLeft />
              ) : (
                <IconPinDown />
              )}
            ...
            <div
              ...
              onClick={() => {
                onRemove(selectedDescriptorsUUIDs);
              }}
            >
              <IconTrash />
            </div>
          ...
          <div className="filterButtons">
            <div
              className={
                "add button" + (addAllowed ? " allowed" : " disallowed")
              }
              onClick={void this.getDescriptor}
            >
              <IconPlus /> Add
            </div>
            <div
              className={
                "listenerSwitch button" +
                (autoUpdateListener ? " activated" : " deactivated")
              }
              onClick={void this.attachListener}
            >
              {autoUpdateListener ? <IconMediaStop /> : <IconMediaRecord />}
              Listener
            </div>
            {
              ...
              Main.isFirstParty && (
                <div
                  className={
                    "listenerSwitch button" +
                    (autoUpdateSpy ? " activated" : " deactivated")
                  }
                  onClick={void this.attachSpy}
                >
                  {autoUpdateSpy ? <IconMediaStop /> : <IconMediaRecord />}Spy
                </div>
              )
            }
            <div
              className={
                "autoInspectorSwitch button" +
                (autoUpdateInspector ? " activated" : " deactivated")
              }
              onClick={void this.attachAutoInspector}
            >
              {autoUpdateInspector ? <IconMediaStop /> : <IconMediaRecord />}
              Inspector
            </div>
          </div>
        </div>
...
> = (dispatch: Dispatch): ILeftColumnDispatch => ({
  ...
  setRenameMode: (uuid: string, on: boolean) =>
    dispatch(setRenameModeAction(uuid, on)),
  onSelect: (operation: TSelectDescriptorOperation, uuid?: string) =>
    dispatch(selectDescriptorAction(operation, uuid)),

  onSetDontShowMarketplaceInfo: (enabled: boolean) =>
    dispatch(setDontShowMarketplaceInfoAction(enabled)),
...
export const LeftColumnContainer = connect<
  ILeftColumnProps,
  ILeftColumnDispatch,
  Record<string, unknown>,
  IRootState
