...
import {
  getModeTabID,
  getActiveDescriptorOriginalReference,
  getFontSizeSettings,
  getLeftColumnWidth,
  getRightColumnWidth,
  getSettingsVisible,
...
class Inspector extends React.Component<TInspector, IInspectorState> {
  ...
  public componentDidMount(): void {
    ...
    (async () => {
      const res = await fetch(
        "http://alchemist.bereza.cz/alchemist-message.json",
      );
      if (res.status !== 200) {
        return;
      }
  ...
  public render(): JSX.Element {
    ...
    const btnSettings = (
      <div
        className={"FilterButton settings " + (visible ? "on " : "off ")}
        title="Show settings"
      ...
      <div className={`Inspector ${fontSizeSettings}`} key={fontSizeSettings}>
        <div className="descriptorsColumns">
          <SplitPane
            className="split"
            split="vertical"
            defaultSize={leftColumnWidthPx}
            onDragFinished={(px) => setColumnSize(px, "left")}
            minSize={200}
          >
        ...
        {this.state.showMessage && (
          <div className="messageStrip">
            <a href={this.state.link} className="link">
              {this.state.message}
            </a>
            <span className="close" onClick={this.closeMessage}>
              <IconX />
            </span>
          </div>
        )}
...
const mapStateToProps = (state: any): IInspectorProps => (
