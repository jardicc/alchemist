...
class DescriptorItem extends React.Component<TDescriptorItem, IState> {
  ...
  private Icon = (): JSX.Element => {
    ...
    return <div className="titleIcon">{icon}</div>;
  };
  ...
  private renderEditState = () => {
    ...
    return (
      <div
        className={"editMode " + this.generateClassName()}
        onClick={this.select}
      >
        <sp-textfield
          class="renameInput"
        ...
        />
        <div className="button" onClick={this.rename}>
          OK
        </div>
        <div className="button" onClick={this.cancel}>
          ×
        </div>
      </div>
    );
  ...
  private renderNormalState = () => {
    ...
    const {
      descriptor: { locked, pinned, groupCount },
    } = this.props;
    return (
      <div
        className={"normalMode " + this.generateClassName()}
        onClick={this.select}
      >
        <this.Icon />
        ...
        {descriptor.startTime === 0 ? null : (
          <div className="time">
            {descriptor.endTime - descriptor.startTime} ms
          </div>
        )}
      </div>
    );
...
const mapDispatchToProps = (dispatch: Dispatch): IDescriptorItemDispatch => ({
  onSelect: (
    ...
    operation: TSelectDescriptorOperation,
    crc?: number,
  ) => dispatch(selectDescriptorAction(operation, uuid, crc)),
  onChangeName: (uuid: string, name: string) =>
    dispatch(renameDescriptorAction(uuid, name)),
  setRenameMode: (uuid: string, on: boolean) =>
    dispatch(setRenameModeAction(uuid, on)),
});

export const DescriptorItemContainer = connect<
  IDescriptorItemProps,
  IDescriptorItemDispatch,
  IOwn,
  IRootState
>(
  mapStateToProps,
