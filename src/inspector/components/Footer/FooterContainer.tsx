...
class Footer extends React.Component<TFooter, Record<string, unknown>> {
  ...
  public render(): React.ReactNode {
    ...
    const uxpVersion =
      valid(coerce(versions?.uxp?.split?.("-")?.[1])) ?? "Parser error";
    const psVersion =
      valid(coerce(psVersionSegments?.split?.(" ")?.[0])) ?? "Parser error";
    const buildParts = psVersionSegments?.split(" ")?.[1]?.split(".");
    const buildNumber =
      buildParts?.length >= 3 ? buildParts[1] + buildParts[2] : "";
      ...
      <div className="Footer">
        <div className="versionBar">
          <span className="version">
            <a href="https://github.com/jardicc/alchemist/releases">
              {versions.plugin} {mode}
            </a>
          </span>
          <span> / </span>
          <span className="version">
            <a href="https://developer.adobe.com/photoshop/uxp/2022/ps_reference/changelog/">
              PS: {psVersion}
            </a>{" "}
            ({buildNumber})
          </span>
        ...
        <div className="copy">
          ...
          <a href="https://bereza.cz">Bereza.cz</a>
        </div>
      </div>
    );
  }
