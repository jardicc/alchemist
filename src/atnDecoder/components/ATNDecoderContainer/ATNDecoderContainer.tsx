...
} from "../../../inspector/model/types";
import {
  getAllDescriptors,
  getFontSizeSettings,
  getInspectorSettings,
} from "../../../inspector/selectors/inspectorSelectors";
import {
  getActionByUUID,
  getData,
  getDontSendDisabled,
...
class ATNDecoder extends React.Component<TATNDecoder, IATNDecoderState> {
  ...
  private renderSet = () => {
    ...
    return data.map((set, i) => <ActionSetContainer actionSet={set} key={i} />);
  };
  ...
  private pass = async (replace = false) => {
    ...
    let {
      selectedCommands,
      onPassSelected,
      onSelectAlchemistDescriptors,
      dontSendDisabled,
      allAlchemistDescriptors,
      settingsAlchemist,
    } = this.props;
    ...
    if (selectedCommands.length > settingsAlchemist.maximumItems) {
      await alert(
        `Alchemist can currently show only ${
          settingsAlchemist.maximumItems
        } items. Increase limit in Alchemist settings if you want to see ${
          selectedCommands.length - settingsAlchemist.maximumItems
        } additional items`,
      );
    }

    selectedCommands.forEach((command, index) => {
      const commandParrent = getActionByUUID(
        rootStore.getState().inspector,
        command.__uuidParentSet__,
        command.__uuidParentAction__,
      );
      ...
      const desc: IDescriptor = {
        ...
        startTime: 0,
        title:
          PS.core.translateUIString(commandParrent?.actionItemName ?? "") +
          " / " +
          command.commandName,
      };
  ...
  public render(): JSX.Element {
    const {
      ...
      selectedCommands,
      setSelectedItem,
      onSetDontSendDisabled,
      dontSendDisabled,
    } = this.props;

    return (
      <div
        className={`ATNDecoderContainer ${fontSizeSettings}`}
      ...
      >
        <div className="info spread flex">
          ...
          <div className="atnCode">
            <SP.Textarea className="infoBlock" value={textData} />
          </div>
        </div>
        <div className="buttonBar">
          ...
          <div
            className={
              "button " + (!selectedCommands.length ? "disallowed" : "")
            }
            onClick={() => void this.pass()}
          >
            Add to Alchemist
          </div>
          <div
            ...
            onClick={() => void this.pass(true)}
          >
            Replace in Alchemist
          </div>
          <SP.Checkbox
            onChange={() => onSetDontSendDisabled(!dontSendDisabled)}
            checked={dontSendDisabled}
          >
            {"Don't send disabled"}
          </SP.Checkbox>
          <div className="spread"></div>
          <div
            className={"button " + (!data.length ? "disallowed" : "")}
            onClick={onClearAll}
          >
            Clear all
          </div>
...
const mapStateToProps = (state: IRootState): IATNDecoderProps => (
...
> = (dispatch): IATNDecoderDispatch => ({
  ...
  onSelectAlchemistDescriptors: (operation, uuid) =>
...
export const ATNDecoderContainer = connect<
  IATNDecoderProps,
  IATNDecoderDispatch,
  Record<string, unknown>,
  IRootState
