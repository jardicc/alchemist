...
export class ActionSet extends React.Component<TActionSet, IActionSetState> {
  ...
  private get isSelected(): boolean {
    ...
    const found = selectedItems.find((item) => item[0] === uuids[0]);
      ...
      <div className="ActionSet">
        ...
        >
          <div className="checkmark">
            {actionSet.actionItems?.every(
              (aItem) => aItem.commands?.every((item) => item.enabled) ?? true,
            ) ?? true ? (
              <IconCheck />
            ) : (
              <IconEmpty />
        ...
        {this.isExpanded &&
          actionSet.actionItems.map((item, index) => (
            <ActionItemContainer
              actionItem={item}
              parent={actionSet}
              key={actionSet + "_" + item + "_" + index}
            />
          ))}
      </div>
    );
...
const mapDispatchToProps = (dispatch: Dispatch): IActionSetDispatch => ({
  setExpandedItem: (uuid, expand, recursive) =>
    dispatch(setExpandActionAction(uuid, expand, recursive)),
  setSelectedItem: (uuid, operation) =>
    dispatch(setSelectActionAction(operation, uuid)),
});

export const ActionSetContainer = connect<
  IActionSetProps,
  IActionSetDispatch,
  IOwn,
  IRootState
