...
export const all = (state: IRootState): IATNConverterState =>
  state.inspector.atnConverter;
export const getData = createSelector([all], (s) => s.data);

export const getSetByUUID = (
  state: IInspectorState,
  uuidSet: string,
): IActionSetUUID | null => {
  const res =
    state.atnConverter.data.find((set) => set.__uuid__ === uuidSet) || null;
  return res;
};

export const getActionByUUID = (
  state: IInspectorState,
  uuidSet: string,
  uuidAction: string,
): IActionItemUUID | null => {
  const res =
    getSetByUUID(state, uuidSet)?.actionItems.find(
      (item) => item.__uuid__ === uuidAction,
    ) || null;
  return res;
};

export const getCommandByUUID = (
  state: IInspectorState,
  uuidSet: string,
  uuidAction: string,
  uuidCommand: string,
): IActionCommandUUID | null => {
  const res =
    getActionByUUID(state, uuidSet, uuidAction)?.commands.find(
      (item) => item.__uuid__ === uuidCommand,
    ) || null;
  return res;
};

export function getTreePartUniversal(
  state: IInspectorState,
  path: [string],
): IActionSetUUID;
export function getTreePartUniversal(
...
export function getTreePartUniversal(
  state: IInspectorState,
  path: string[],
): IActionSetUUID | IActionItemUUID | IActionCommandUUID | null {
  switch (path.length) {
    case 1:
...
export const selected = createSelector(
  [selectedSets, selectedActions, selectedCommands],
...
export const getLastSelected = createSelector(
  [selectedSets, selectedActions, selectedCommands, all],
  (sets, actions, commands, all) => {
    ...
    if (len === 3) {
      return (
        commands.find((s) => s.__uuid__ === all?.lastSelected?.[2]) || null
      );
    }
    return null;
  },
  ...
  (all, lastOne) => {
    ...
    if (!all.lastSelected) {
      ...
      const res = stringifyObject(all.data, {
        ...
        filter: (input, prop) =>
          !(prop === "__uuid__") &&
          !(prop === "__uuidParentSet__") &&
          !(prop === "__uuidParentAction__"), // get rid of  __uuid__ added by occultist
      });
      return res;
